...
import { useScroll, useTransform, useSpring, motion, useInView, AnimatePresence } from 'motion/react';
...
const FeatureListBlockClient: React.FC<FeatureListProps> = ({ accentColor, features, buttons }) => {
    ...
    useEffect(() => {
        const observeFeatureVisibility = () => {
            if (!featureSectionRefs.current) { return };
            ...
            const mostVisibleFeature = featureSectionRefs.current.reduce<{
                ...
                if (!currentFeature) { return mostVisible };
                ...
                return !mostVisible || visibilityPercentage > (mostVisible?.visibilityPercentage || 0)
                    ? { element: currentFeature, visibilityPercentage, id: currentFeature.id || null }
                    : mostVisible;
            }, null);
    ...
    const y = useSpring(
        useTransform(scrollYProgress, [0.1, 0.9], [-100, -50]),
                            ...
                            {features.map((feature, i) => (
                                ...
                                >
                                    <FeatureIcon
                                        ...
                                        iconClasses={`${currentSection === feature.id ? 'stroke-z-' + accentColor : 'stroke-[#5A5A5A]'
                                            }`}
                                        width="24px"
                                    />
                ...
                {features.map((feature, i) => (
                    ...
                    >
                        <div className="flex flex-col gap-2.5 md:gap-5 md:flex-1 md:py-24">
                            <div className="flex flex-col gap-2.5 md:gap-3">
                                <div className="flex gap-2 items-center">
                                    <FeatureIcon color="#919191" height="20px" icon={feature.icon || 'banana'} width="20px" />
                        ...
                        <div
                            ...
                            style={feature.offset ? illustrationMargin(feature.offset) : undefined}
                        >
                            <ImageMedia imgClassName="w-full h-auto" resource={feature.illustration} />
                        </div>
                    </div>
                ))}
            </div>
